  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (!userId || !userToken) {
          throw new Error('ID de usuário ou token não encontrado no localStorage.');
        }
      ...
      <div className="d-flex flex-column align-items-start justify-content-between px-3 pt-4">
        {expanded ? (
          <>
            ...
            <ButtonLer className="LerMais" variant="Link" onClick={() => setExpanded(false)}>Mostrar Menos</ButtonLer>
          ...
          <>
            ...
            <ButtonLer variant="Link" onClick={() => setExpanded(true)} className="text-start">Mostrar Mais...</ButtonLer>
          </>
        )}
